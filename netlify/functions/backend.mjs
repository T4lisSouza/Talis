...
export default async function handler(req) {
  try {
    ...
    if (parts[0] === "setup") return setup(req, body);
    if (parts[0] === "login") return login(req, body);
    if (parts[0] === "logout") return logout(req);
    if (parts[0] === "register") return register(req, body);
    if (parts[0] === "password") return changePassword(req, body);
    if (parts[0] === "upload") return upload(req, body);
    if (parts[0] === "settings") return saveSettings(req, body);
    if (parts[0] === "products") return productAction(req, parts[1], body);
    if (parts[0] === "users") return userAction(req, parts[1], body);
    if (parts[0] === "orders") return orderAction(req, parts[1], body);
...
function cookies(header) {
  return Object.fromEntries(header.split(";").map((p) => p.trim()).filter(Boolean).map((p) => [p.slice(0, p.indexOf("=")), decodeURIComponent(p.slice(p.indexOf("=") + 1))]));
}
